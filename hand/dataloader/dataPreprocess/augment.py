...
def processing_augmentation(
        image, depth, cloud, heatmap, pose3d, hand_side, uv, mask, object_mask):
    # flipping. Note that the wrong direction here will make the 3D EPE around 50!!!
    # Adjust the ordering of it in the data preprocessing.
    if (hand_side[0] == 1.0):
        ...
        cloud[:, 0] = -cloud[:, 0]
        pose3d[:, 0] = -pose3d[:, 0]
        uv[:, 0] = 255 - uv[:, 0]  # uv_aug
        mask = cv2.flip(mask, 1)
        object_mask = cv2.flip(object_mask, 1)
    ...
    mask = cv2.warpAffine(mask, rotMat, (256, 256), flags=cv2.INTER_NEAREST, borderValue=0.0)
    object_mask = cv2.warpAffine(object_mask, rotMat, (256, 256), flags=cv2.INTER_NEAREST, borderValue=0.0)
    ...
    uv_aug = np.reshape(uv_aug,[42])

    return image, depth, cloud, heatmap, pose3d, uv_aug, rotMat, mask, object_mask

def processing(image, depth, cloud, heatmap, pose3d, hand_side, uv, mask, object_mask):
    randInidices = np.arange(len(cloud))
    np.random.shuffle(randInidices)
    cloud = cloud[randInidices[0:POINT_SIZE, ], :]
    ...
    return image, depth, cloud, heatmap, pose3d, uv, mask, object_mask
