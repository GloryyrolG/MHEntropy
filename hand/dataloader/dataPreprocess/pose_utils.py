...
def vis_keypoints(img: np.ndarray, kps: np.ndarray, kps_lines, kp_thresh=0.4,
                  alpha=1) -> np.ndarray:
    """
    Args:
        img: np.uint8
        kps: shape: (3/2, K) or (K, 3/2)
    """
    if kps.shape[0] > 3:  # shape: (K, 3)
        kps = kps.transpose((1, 0))
    if kps.shape[0] == 2:  # shape: (2, K)
        kps = np.concatenate((kps, np.ones((1, kps.shape[1]), dtype=kps.dtype)),
                             axis=0)
