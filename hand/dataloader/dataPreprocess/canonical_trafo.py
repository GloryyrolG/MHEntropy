...
def atan2(y, x):
    ...
    one_map = np.ones_like(tan)

    # correct quadrant error
    correction = np.where(np.less(x + 1e-8, 0.0), 3.141592653589793*one_map, 0.0*one_map)
    tan_c = tan + correction  # this returns in -pi/2 .. 3pi/2

    # bring to positive values
    correction = np.where(np.less(tan_c, 0.0), 2*3.141592653589793*one_map, 0.0*one_map)
    tan_zero_2pi = tan_c + correction  # this returns in 0 .. 2pi

    # make symmetric
    correction = np.where(np.greater(tan_zero_2pi, 3.141592653589793), -2*3.141592653589793*one_map, 0.0*one_map)
...
def _stitch_mat_from_vecs(vector_list):
    ...
    batch_size = 1
    vector_list = [np.reshape(x, (1, batch_size)) for x in vector_list]

    # trafo_matrix = tf.dynamic_stitch([[0], [1], [2],
    #                                   [3], [4], [5],
    #                                   [6], [7], [8]], vector_list)
    trafo_matrix = np.array(vector_list).reshape(9,1)

    trafo_matrix = np.reshape(trafo_matrix, (3, 3, batch_size))
    trafo_matrix = np.transpose(trafo_matrix, (2, 0, 1))
...
def _get_rot_mat_x(angle):
    ...
    one_vec = np.ones_like(angle)
    zero_vec = one_vec*0.0
    trafo_matrix = _stitch_mat_from_vecs([one_vec, zero_vec, zero_vec,
                                          zero_vec, np.cos(angle), np.sin(angle),
                                          zero_vec, -np.sin(angle), np.cos(angle)])
    return trafo_matrix
...
def _get_rot_mat_y(angle):
    ...
    one_vec = np.ones_like(angle)
    zero_vec = one_vec*0.0
    trafo_matrix = _stitch_mat_from_vecs([np.cos(angle), zero_vec, -np.sin(angle),
                                          zero_vec, one_vec, zero_vec,
                                          np.sin(angle), zero_vec, np.cos(angle)])
    return trafo_matrix
...
def _get_rot_mat_z(angle):
    ...
    one_vec = np.ones_like(angle)
    zero_vec = one_vec*0.0
    trafo_matrix = _stitch_mat_from_vecs([np.cos(angle), np.sin(angle), zero_vec,
                                          -np.sin(angle), np.cos(angle), zero_vec,
...
def canonical_trafo(coords_xyz):
    ...
    """
    coords_xyz = np.reshape(coords_xyz, (-1, 21, 3))
    ...
    trans = np.expand_dims(coords_xyz[:, ROOT_NODE_ID, :], 1)
...
def flip_right_hand(coords_xyz_canonical, cond_right):
    ...
    if len(s) == 2:
        coords_xyz_canonical = np.expand_dims(coords_xyz_canonical, 0)
        cond_right = np.expand_dims(cond_right, 0)
        expanded = True

    # mirror along y axis
    coords_xyz_canonical_mirrored = np.stack([coords_xyz_canonical[:, :, 0], coords_xyz_canonical[:, :, 1], -coords_xyz_canonical[:, :, 2]], -1)

    # select mirrored in case it was a right hand
    coords_xyz_canonical_left = np.where(cond_right, coords_xyz_canonical_mirrored, coords_xyz_canonical)

    if expanded:
        coords_xyz_canonical_left = np.squeeze(coords_xyz_canonical_left, axis=0)
